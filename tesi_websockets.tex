...
I Server Sent Events (SSE) sono una tecnologia push che permette ad un broswer di ricevere aggiornamenti da un server tramite una connessione HTTP. L'API che offre questa funzionalità, chiamata EventSource API, è standardizzata dal W3C come parte di HTML5.

Il flusso di una comunicazione basata su \textit{EventSource} è il seguente:
...
La limitazione di questa tecnologia è che una volta aperta una connessione, il client non potrà sfruttare la stessa per inviare ulteriori messaggi. Ciò significa che per rimanere in ascolto di diverse risorse in instanti differenti, sarà necessario aprire più connessioni.
